...
var nearestPriorBin = function(x){
  return priorBins[Math.round(((x - minBin)/(maxBin - minBin))*priorBins.length)]
}
...
var discretizeLogNormalPrior = function(e, mu, sigma){
  Enumerate(function(){
    return flip(e) ? priorBins[discrete(gaussianPMF(mu, sigma, priorBins))] : maxBin
  })
}
...
var discretizeLogNormalPrior2 = function(v, bins){
  ...
  var sigma = v[2]
  Enumerate(function(){
    return flip(e) ? bins[discrete(gaussianPMF(mu, sigma, bins))] : maxBin
  })
}
