...
In this paper we present the first empirical data on the felicity of habitual sentences and we describe a formal model of habitual interpretation, based on recent Bayesian models of the pragmatics of vague language.
...
Is the underlying dimension for habituals the actual, or past, frequency of the event (e.g.~how often Bill has actually smoked in the past month) or the predictive probability that this event will happen in the near future (e.g.~how likely it is that Bill will smoke in the next week)?
...
For a given individual $I$ (e.g.~\textsc{Bill}) and an event or behavior $E$ (e.g.~\textsc{smoking}), we refer to the rate at which an individual $I$ participates in an event $E$ for a given time window $t$, that is $ \lambda_{t} = \int_{t} \mathbbm{1}_{I \in E}dt$, as the \emph{propensity} of $I$ doing $E$.
%\ndg{maybe it's better to talk about the underlying scale as a rate? what's up with the ** in the integral here?}
%
A natural denotation for the habitual is a simple threshold on propensity $\lambda_{t} > \tau$ \cite<c.f.>{Cohen1999}, yet no fixed value for $\tau$ would lead to the observed flexibility of truth conditions (e.g. \emph{Bill smokes} vs. \emph{writes novels}).
...
We model a speaker $S_2$ who reasons about a pragmatic listener $L_1$; this listener is considering the propensity of a certain behavior for a certain individual, $\lambda$.
...
In Expts.~1\&2, we begin to address this issue.
...
Question 2 elicits the relative frequency that a person does the action.
...
On each trial, participants were presented with a \emph{past frequency statement} for a given event or behavior of the form: ``In the past M \{weeks, months, years\}, \textsc{person} \textsc{did x} 3 times''.
For example, \emph{In the past month, Bill smoked cigarettes 3 times}.
\mht{Should we add: Because it is a matter of debate what constitutes a discrete event (cite someone who does event segmentation), we intentionally kept the frame of \textsc{did x} 3 times, as opposed to e.g. smoked 3 cigarettes.}
...
We observe no difference between endorsements of the habitual of characters with male and female names, and overall, the mean endorsements by gender are heavily correlated $r(93) = 0.91$. 
...
Less interestingly, the lack of a difference may be the result of gender being not very salient in our paradigm.
...
\section{Experiment 3: Observed frequency versus expectation}
...
In this set of experiments, we address this by introducing causal factors that enable or prevent future actions (e.g. buying a pack of cigarettes; developing an allergy).
In Expt.~3a, we measure \emph{predictive frequency} when these causal factors are introduced as well as when just a past frequency is observed.
...
This will allow us to test whether habituals are best explained by as a speaker $S_2$ who wants to communicate past or \emph{predictive} frequency.
...
%  \includegraphics[width=\textwidth]{truth-judgments-3items-withtj2.pdf}
  ...
  \caption{Left: Predicted log frequency as a function of past log frequency supplied to the participant (Expt.~3a).
...
Figure \ref{fig:tj3} (middle) shows the predicted future frequency as a function of the past frequency given to the participant and the type of causal information given (in color; confidence intervals suppressed and jitter added for visual clarity). 
...
Critically, we observe the preventative information appreciably decreasing and the enabling information slightly increasing predicted frequency (Figure \ref{fig:tj3} middle; blue and green dots).
...
This may be due, in part, to an effect of the experiment context on participants: Participants are viewing extra information that strongly disables the action from occurring in may cases; when that information is not present, participants may be more inclined to endorse the habitual.
This may the be result of an inferential process about the experimenter's beliefs on behalf of the participant: Because the distribution of frequencies in this task was more limited than those used in Expt.~2, the participant may infer that the experimenter believes this to be a representative range and adjust her judgments accordingly.
...
The same model using the past frequency as the object of communication matches the data poorly ($r^2(63) = 0.02$).
These results suggest that the felicity of habituals is based on an underlying scale of predicted future propensity, or frequency, not the mere observed frequency in the past.


\section{Discussion}

We presented a computational model for communicating generalizations about events.
The model decides if a habitual is a useful way to describe a person's behavior, taking into account the listener's prior beliefs about the action --- how common it is and the likely frequency (measured in Expt.~1) --- and the basic conversational principles to be truthful and informative.
We validated this model by eliciting felicity judgments for habitual sentences covering a wide range of activities with a wide range of experimentally manipulated frequencies of action (Expt.~2).
...
To our knowledge, the experiments presented here are first empirical investigations into the truth conditions of habitual sentences and the first test of a formal model of habitual sentences.
...
The computational model presented here provides a computational bridge between the way we talk about people's behavior and our intuitive theories of others and events.
