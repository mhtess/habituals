...
To attain credible values of the model parameters as well as the posterior predictive distribution over responses, we collected \red{X} samples using an MCMC algorithm...
The Maximum A-Posteriori (MAP) value and 95\% highest probability density (HPD) interval for the speaker optimality parameter in Eq.~\ref{eq:S1} is \red{X [A, B]}.
The MAP and HPD interval for the data-analytic guessing parameter is \red{X [A, B]}.

As a baseline hypothesis, we see how the frequency of action predicts the felicity of a habitual sentence (e.g. how often John runs to predict the felicity of the sentence \emph{John runs}.). Figure \ref{fig:tjScatters} (left) shows the correspondence between the frequency of action --- plotted as the log time interval, the time between subsequent actions --- and the felicity of the corresponding habitual sentence. 
It is clear that a habitual sentence can receive strong agreement even when the actions are very infrequent (e.g. writing 3 novels, stealing 3 cars in a 5 year interval).
...
The probabilistic pragmatics model does a better job of accounting for the variability in responses ($r^2(93) = 0.85$), including actions done on the time scale of years or more  ($r^2(50) = 0.84$).
