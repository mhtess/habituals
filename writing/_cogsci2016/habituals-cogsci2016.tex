...
Figuring out that a person tends to exhibit a behavior can be crucial and hard-won knowledge.
...
Is the underlying dimension for habituals the actual, or past, frequency of the event (e.g.~how often Bill has actually smoked in the past week) or the predictive probability that this event will happen in the near future (e.g.~how likely it is that Bill will smoke in the next week)?
...
For a given individual $I$ (e.g.~\textsc{Bill}) and an event or behavior $E$ (e.g.~\textsc{smoking}), we refer to the rate at which an individual $I$ participates in an event $E$ for a given time window  
